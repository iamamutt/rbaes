...
#'         is.null(data$ymax) && is.null(self$height)) {
...
#' )
#'
#' stat_posterior_density <- function(
#'   mapping = NULL,
#'   data = NULL,
#'   geom = "posterior",
#'   position = "spread",
...
#'     dens$height <- height
#'
#'     interval_frame <- make_post_int_frame(data$x, center_stat, interval_width, interval_type)
...
#'   {
#'     data$height <- data$height %||% params$height %||% (resolution(data$y, FALSE) * 0.9)
#'
#'     # ymin, ymax, xmin, and xmax define the bounding rectangle for each group
#'     plyr::ddply(data, "group", transform, ymin = y - height/2, ymax = y + height/2)
#'   },
#'   draw_group = function(self, data, ...) {
#'
#'     interp_dens_y <- function(xdens, ymin, ymax, x) {
#'       c(stats::approxfun(xdens, ymin)(x),
...
#'         y = interp_dens_y(post_trace$x, post_trace$yminv, post_trace$ymaxv, vlinectr),
#'         group = c(vlinectr, vlinectr))
#'
#'       aesthetics <- post_trace[rep(1, nrow(post_vline)), setdiff(names(post_trace), c("x", "y", "group")), drop = FALSE]
...
#'       post_segc <- trace_violin(data, data$segxminc[1], data$segxmaxc[1])
...
#'       post_segs <- trace_violin(data, data$segxmins[1], data$segxmaxs[1])
...
#'     }
#'
#'     ggname("geom_posterior", grid::grobTree(
#'       post_fill_grob,
#'       segc_grob,
#'       segs_grob,
#'       vline_grob,
...
#' }
#'
#' make_post_int_frame <- function(
#'   x,
#'   center_stat = NULL,
#'   interval_width = NULL,
#'   interval_type = 'hdi')
#' {
#'   p_int_frame <- data.frame(
#'     vlinectr = NA,
#'     segxmins = NA,
#'     segxmaxs = NA,
#'     segxminc = NA,
...
#'   )
#'
#'   no_ctr <- is.null(center_stat)
#'   no_ci <- is.null(interval_width)
#'
#'   if (no_ctr & no_ci) {
#'     return(p_int_frame)
#'   }
...
#'
#'   if (!no_ctr) {
#'     p_int_frame$vlinectr <- markers[markers$type == 'mid', 'y']
#'   }
#'
#'   if (!no_ci) {
#'     std <- markers[markers$type == 'std',]
#'     cint <- markers[markers$type == interval_type,]
#'     p_int_frame$segxmins = std[std$side == 'l', 'y']
#'     p_int_frame$segxmaxs = std[std$side == 'r', 'y']
#'     p_int_frame$segxminc = cint[cint$side == 'l', 'y']
#'     p_int_frame$segxmaxc = cint[cint$side == 'r', 'y']
#'   }
#'   return(p_int_frame)
#' }
#'
